...
#ifndef GIL_FILE_HELPER_H
#define GIL_FILE_HELPER_H
...
namespace detail {
   ...
   typedef unsigned char byte_t;
    ...
    //////////////////////////
    /// RAII class, plus a number
    /// of extra formatted io functions
    ///
    class file_helper {
    ...
    protected:
        ...
        file_helper(FILE* file) : _fp(file, null_deleter()) {}

        file_helper(const char* filename, const char* flags) {
            FILE* fp;
            io_error_if((fp=fopen(filename,flags))==NULL, "file_helper: failed to open file");
            _fp=boost::shared_ptr<FILE>(fp,fclose);
        }

        file_helper(const wchar_t* filename, const wchar_t* flags) {
            FILE* fp;
            #ifdef _WIN32
               io_error_if((fp=_wfopen(filename,flags))==NULL, "file_helper: failed to open file");
            #else
               ...
               // open file
               io_error_if((fp=fopen(filename_buf,flags_buf))==NULL, "file_helper: failed to open file");
            #endif
