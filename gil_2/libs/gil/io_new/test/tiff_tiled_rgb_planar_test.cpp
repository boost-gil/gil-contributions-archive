...
#define GENERATE_TILE_STRIP_COMPARISON_RGB_PLANAR_BIT_ALIGNED(z, n, data)\
    BOOST_AUTO_TEST_CASE( BOOST_PP_CAT(BOOST_PP_CAT(read_tile_and_compare_with_rgb_strip_planar_,n), bit_bit_aligned) )\
    { \
      std::string filename_strip( tiff_in_GM + "tiger-rgb-strip-planar-" ); \
      std::string filename_tile ( tiff_in_GM + "tiger-rgb-tile-planar-"  ); \
...
BOOST_PP_REPEAT_FROM_TO(1, 33, GENERATE_TILE_STRIP_COMPARISON_RGB_PLANAR_BIT_ALIGNED, ~)
GENERATE_TILE_STRIP_COMPARISON_RGB_PLANAR_BIT_ALIGNED(dummy, 64, dummy);
