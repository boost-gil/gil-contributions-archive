...
#ifndef GIL_APPLY_OPERATION_BASE_IO_NEW_HPP
#define GIL_APPLY_OPERATION_BASE_IO_NEW_HPP
...
GENERATE_APPLY_FWD_OPS generates for every N functions that look like this (for N==2):
    ...
    template <> struct apply_operation_fwd_fn<3> {
        ...
        typename UnaryOp::result_type apply(Bits& bits, std::size_t index, const Info& info, UnaryOp op) const {
            ...
            typedef typename mpl::next<T1>::type T2;
            switch (index) {
                case 0: return op(reinterpret_cast<typename mpl::deref<T0>::type&>(bits), info);
                case 1: return op(reinterpret_cast<typename mpl::deref<T1>::type&>(bits), info);
                case 2: return op(reinterpret_cast<typename mpl::deref<T2>::type&>(bits), info);
            }
            throw;
        }
        ...
        typename UnaryOp::result_type applyc(const Bits& bits, std::size_t index, const Info& info, UnaryOp op) const {
            ...
            typedef typename mpl::next<T1>::type T2;
            switch (index) {
                case 0: return op(reinterpret_cast<const typename mpl::deref<T0>::type&>(bits), info);
                case 1: return op(reinterpret_cast<const typename mpl::deref<T1>::type&>(bits), info);
                case 2: return op(reinterpret_cast<const typename mpl::deref<T2>::type&>(bits), info);
            }
            throw;
        }
    };
...
#define GIL_FWD_CASE_CHH(z, N, SUM)       case N: return op(*gil_reinterpret_cast<typename mpl::deref<T##N>::type*>(&bits), info);
#define GIL_FWD_CONST_CASE_CHH(z, N, SUM) case N: return op(*gil_reinterpret_cast_c<const typename mpl::deref<T##N>::type*>(&bits), info);
...
#define GIL_APPLY_FWD_OP(z, N, text)                                                                        \
    template <> struct apply_operation_fwd_fn<BOOST_PP_ADD(N,1)> {                                      \
        ...
        typename UnaryOp::result_type apply(Bits& bits, std::size_t index, const Info& info, UnaryOp op) const {        \
            ...
            T##N;                                                                                       \
            switch (index) {                                                                            \
                BOOST_PP_REPEAT(BOOST_PP_ADD(N,1), GIL_FWD_CASE_CHH, BOOST_PP_EMPTY)                            \
            }                                                                                           \
            throw;                                                                                      \
        }                                                                                               \
        ...
        typename UnaryOp::result_type applyc(const Bits& bits, std::size_t index, const Info& info, UnaryOp op) const { \
            ...
            T##N;                                                                                       \
            switch (index) {                                                                            \
                BOOST_PP_REPEAT(BOOST_PP_ADD(N,1), GIL_FWD_CONST_CASE_CHH,BOOST_PP_EMPTY)                       \
            }                                                                                           \
            throw;                                                                                      \
        }                                                                                               \
    };
