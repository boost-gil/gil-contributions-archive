...
struct row_buffer_helper
{
    ...
    typedef typename image< Pixel >::view_t view_t;
    ...
    view_t create_view( std::ptrdiff_t width
                      , std::ptrdiff_t height
                      )
    {
        typedef typename view_t::x_iterator x_iterator;

        std::size_t byte_to_memunit_value = byte_to_memunit< x_iterator >::value;
        std::size_t size_in_memunits  = width * memunit_step( x_iterator() );
        std::size_t row_size_in_bytes = ( size_in_memunits + byte_to_memunit_value - 1 ) / byte_to_memunit_value;

        return interleaved_view( width
                               , height
                               , data()
                               , row_size_in_bytes
                               );
