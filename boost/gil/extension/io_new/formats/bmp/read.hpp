...
private:
                     ...
                     )
    {
        ...
        if( _info._compression == bmp_compression::_bitfield )
        {
            ...
            mask.red.width   = detail::count_ones( mask.red.mask   );
            mask.green.width = detail::count_ones( mask.green.mask );
            mask.blue.width  = detail::count_ones( mask.blue.mask  );

            mask.red.shift   = detail::trailing_zeros( mask.red.mask   );
            mask.green.shift = detail::trailing_zeros( mask.green.mask );
            mask.blue.shift  = detail::trailing_zeros( mask.blue.mask  );
        }
